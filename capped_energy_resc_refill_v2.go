...
func tophatKernel(x0 float64, y0 float64, R float64, amp float64) float64 {
	// top hat kernel for perception
...
func gaussianKernel(x0 float64, y0 float64, R float64, amp float64) float64 {
	// Gaussian kernel for perception
	// runs for kernel function and probability
	return amp * math.Exp(-math.Sqrt(x0*x0 + y0*y0)/(2*R*R))
}
...
func main() {
	...
	header := "Iteration,X,Y,Type,Energy,Radius\n"
	fmt.Printf(header)
	for iter := 0; iter < iters; iter++ {
		...
		**/
		...
		for i:=0; i < len(agentList); i++ {
			...
			if (len(resourceList) > 0) {
				for j:=0; j < len(resourceList); j++ {
					...
					if (d < a.Radius && !hasClosestResource) {
						...
						closestDist = d
						closestResource = r
					} else if (d < gatherDist && !hasClosestResource) {
						hasClosestResource = true
						closestDist = d
						closestResource = r
					} else if (d < gatherDist && d < closestDist) {
						closestDist = d
						closestResource = r
					}
				}					
				
				resourceInGatherDist = hasClosestResource
			}
			...
			// take cost penalty
			a.Energy -= basalCost + radiusCost*a.Radius
		...
		for i:=0; i < len(agentList); i++ {
			fmt.Printf("%d,%f,%f,agent,%f,%f\n", iter, agentList[i].X, agentList[i].Y, agentList[i].Energy, agentList[i].Radius)
		}
